...
function renderHome() {
  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <h2>Изучай языки в своём темпе.</h2>
      <p>Каждый язык — отдельная папка. Внутри уже есть разделы для слов и для фраз.</p>
    </section>
    <div class="fab-row">
      <button class="btn primary" id="addLanguage">＋ Новый язык</button>
    ...
    ${langs.length ? `<div class="grid languages">${langs.map(languageCard).join('')}</div>` : emptyBlock('Пока нет языков', 'Добавь первый язык — например, казахский или английский.')}
  `;
...
function renderLanguage() {
  ...
  app.innerHTML = `
    ...
    <div class="fab-row">
      <button class="btn primary" id="startStudy">▶ Начать обучение</button>
      <button class="btn secondary" id="renameLanguage">Переименовать</button>
    </div>
    <div class="section-head"><div class="title">Папки</div><div class="count">2</div></div>
    <div class="folder-grid">
      ${folderCard(lang.folders[0], '▤', 'Отдельные слова')}
      ${folderCard(lang.folders[1], '≡', 'Словосочетания и предложения')}
    </div>
    <div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      <button class="btn ghost" id="exportLang">Экспорт языка</button>
      <button class="btn danger" id="deleteLanguage">Удалить язык</button>
    </div>
  `;
  ...
  $('#startStudy').onclick = () => {
    ...
    if (!all.length) return toast('Сначала добавь хотя бы одну карточку');
    openStudy(all);
  };
...
function renderFolder() {
  ...
  app.innerHTML = `
    <div class="fab-row">
      <button class="btn primary" id="addCard">＋ Новая карточка</button>
      <button class="btn secondary" id="studyFolder">▶ Учить</button>
    </div>
    ...
    ${cards.length ? `<div class="cards-list">${cards.map(cardHtml).join('')}</div>` : emptyBlock('Карточек пока нет', 'Добавь слово или фразу с переводом.')}
  `;
...
function renderStudy() {
  ...
  app.innerHTML = `
    ...
    <article class="study-card" id="studyCard" aria-label="Нажмите, чтобы показать перевод">
      <div class="study-label">${studyRevealed ? 'ПЕРЕВОД' : 'ВОПРОС'}</div>
      <div class="study-text">${esc(studyRevealed ? card.back : card.front)}</div>
      ${studyRevealed ? '<div class="study-back">Нажми «Далее» для следующей карточки.</div>' : '<div class="study-back">Нажми на карточку, чтобы открыть перевод.</div>'}
    </article>
